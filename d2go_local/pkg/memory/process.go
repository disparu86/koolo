...
func (p Process) getProcessMemory() ([]byte, error) {
	data := make([]byte, p.moduleBaseSize)

	// Try reading the entire module at once first (fast path)
	err := windows.ReadProcessMemory(p.handler, p.moduleBaseAddressPtr, &data[0], uintptr(p.moduleBaseSize), nil)
	if err == nil {
		return data, nil
	}

	// If full read fails, read page by page skipping inaccessible pages
	const pageSize = 4096
	...
	for offset := uintptr(0); offset < totalSize; offset += pageSize {
		...
		windows.ReadProcessMemory(
			p.handler,
			p.moduleBaseAddressPtr+offset,
			&data[offset],
			uintptr(chunkSize),
			nil,
		)
		// Ignore errors - inaccessible pages stay as zeroes
	}
